...
def merge_scores(path, metric, save_path):
	...
	acc_tables.drop(columns=univariate_detector_names, inplace=True)
	...
	df = pd.merge(df, metric_scores[["TRUE_ORACLE-100", "AVG_ENS"] + univariate_detector_names], left_index=True, right_index=True)
	df.rename(columns={'TRUE_ORACLE-100': 'Oracle', 'AVG_ENS': 'Avg Ens'}, inplace=True)
	
	# Add true labels from AUC_PR metrics
	auc_pr_detectors_scores = metricsloader.read('AUC_PR')[univariate_detector_names]
...
def merge_scores_mts(path, metric, save_path, mts_metrics_path, mts_acc_tables_path):
	...
	# Add Oracle (TRUE_ORACLE-100) and Averaging Ensemble
	df = pd.merge(df, metric_scores[["TRUE_ORACLE-100", "AVG_ENS"] + multivariate_detector_names], left_index=True,
				  right_index=True)
	df.rename(columns={'TRUE_ORACLE-100': 'Oracle', 'AVG_ENS': 'Avg Ens'}, inplace=True)
