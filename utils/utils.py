...
def load_data(data_path, window, dataset=None, features=False, split_file=None):
    ...
    if split_file is not None:
        split_file_path = os.path.join(data_path, 'splits',
                                       'unsupervised' if 'unsupervised' in split_file else 'supervised')
        split_file = os.path.join(split_file_path, split_file) + '.csv'

    raw_timeseries, raw_anomalies, timeseries_names, window_timeseries = load_timeseries(raw_path, window_path,
    ...
    if split_file is not None:
        ...
        split_fnames = set([x[:-len('.csv')] for x in split_data.loc['test_set'].tolist() if
...
def load_timeseries(raw_path, window_path, feature_path, datasets=None, features=False):
    ...
    for dataset in datasets:
        ...
        curr_df = dataloader.load_window_timeseries_parallel(
            dataset) if not features else dataloader.load_feature_timeseries(dataset)
...
def compute_metrics(labels, scores, k=None):
    ...
    for label, score in tqdm(zip(labels, scores),
                             desc=f"Computing metrics {k}" if k is not None else "Computing metrics", leave=False,
                             total=len(labels)):
...
def wrapper_get_metrics(score, label):
    metrics_to_keep = ["AUC-ROC", "AUC-PR", "VUS-ROC", "VUS-PR", 'INTERPRETABILITY_HIT_2']
...
def load_model(model_name, window_size, weights_path):
    if any([True for x in ['convnet', 'resnet', 'sit'] if x in model_name]):
        model_parameters_file = os.path.join('src', 'models', 'configuration',
                                             model_name.replace(f"_{window_size}", '.json'))
